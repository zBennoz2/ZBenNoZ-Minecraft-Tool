...
const parseKeyValue = (line: string): { key: string; value: string } | null => {
  ...
  if (trimmed.startsWith('#') || trimmed.length === 0) return null;
...
export const parse = (raw: string): ParsedProperties => {
  const lines: PropertyLine[] = [];
  const props: Record<string, string> = {};

  const splitted = raw.replace(/\r\n/g, '\n').replace(/\r/g, '\n').split('\n');
  ...
  for (const line of splitted) {
    ...
    if (kv) {
      ...
      // Unknown format -> treat as comment to preserve
      lines.push({ type: 'comment', raw: line });
    }
  }
...
export const apply = (raw: string, set: Record<string, string>, unset: string[]): string => {
