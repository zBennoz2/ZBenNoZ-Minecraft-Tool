...
const isHttpsRequest = (req: express.Request) => req.secure || req.header('x-forwarded-proto')?.split(',')[0]?.trim() === 'https'
const setSessionCookie = (req: express.Request, res: express.Response, token: string, expiresAt: string) => { const parts = [`${cookieName}=${encodeURIComponent(token)}`, 'Path=/', 'SameSite=Lax', 'HttpOnly', `Expires=${new Date(expiresAt).toUTCString()}`]; if (isHttpsRequest(req) || process.env.NODE_ENV === 'production') parts.push('Secure'); res.setHeader('Set-Cookie', parts.join('; ')) }
...
const getCurrentSession = async (req: express.Request, res: express.Response) => {
  ...
  if (!localUser) return res.status(401).json({ authenticated: false, error: 'AUTH_REQUIRED', message: 'Login erforderlich.', license })
  return res.json(authPayload(localUser, { license }))
}
