...
import { getSession as getRemoteSession } from '../services/auth.service'
...
const resolveRemoteLicenseAdmin = async () => { const remote = await getRemoteSession(); if (!remote.authenticated) return null; const username = remote.user?.name || remote.user?.email || remote.user?.id; return localUsersService.ensureLicenseAdmin(username) }
export const requireAuth = async (req: Request, res: Response, next: NextFunction) => { const user = await localUsersService.getUserBySession(getSessionToken(req)) || await resolveRemoteLicenseAdmin(); if (!user) return res.status(401).json({ error: 'AUTH_REQUIRED', message: 'Login erforderlich.' }); req.user = user; return next() }
