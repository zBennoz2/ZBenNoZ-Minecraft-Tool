...
export const requireAdmin = (req: Request, res: Response, next: NextFunction) => req.user?.role === 'admin' ? next() : res.status(403).json({ error: 'ADMIN_REQUIRED', message: 'Adminrechte erforderlich.' })
export const requireInstanceAccess = async (req: Request, res: Response, next: NextFunction) => { const id = req.params.id; if (!req.user) return res.status(401).json({ error: 'AUTH_REQUIRED', message: 'Login erforderlich.' }); if (!id || await localUsersService.hasInstanceAccess(req.user, id)) return next(); return res.status(403).json({ error: 'INSTANCE_ACCESS_DENIED', message: 'Keine Berechtigung für diese Instanz.' }) }
