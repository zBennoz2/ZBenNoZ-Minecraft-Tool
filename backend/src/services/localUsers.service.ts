...
const LICENSE_ADMIN_ID = 'license_admin'
const LICENSE_ADMIN_USERNAME = 'Lizenz-Hauptadmin'
const LICENSE_ADMIN_PASSWORD_HASH = 'external:api-license-login'
...
export const localUsersService = {
  ...
  async hasAdmin() { return (await readJson<UsersFile>(usersPath(), { users: [] })).users.some((u) => u.role === 'admin' && !u.disabled) },
  ...
  async ensureLicenseAdmin(username?: string) { return enqueue(async () => { const data = await readJson<UsersFile>(usersPath(), { users: [] }); const existingLicenseAdmin = data.users.find((u) => u.id === LICENSE_ADMIN_ID); if (existingLicenseAdmin) return existingLicenseAdmin; const existingAdmin = data.users.find((u) => u.role === 'admin' && !u.disabled); if (existingAdmin) return existingAdmin; const ts = now(); const preferredUsername = (username || LICENSE_ADMIN_USERNAME).trim() || LICENSE_ADMIN_USERNAME; const uniqueUsername = data.users.some((u) => u.username.toLowerCase() === preferredUsername.toLowerCase()) ? LICENSE_ADMIN_USERNAME : preferredUsername; const user: LocalUser = { id: LICENSE_ADMIN_ID, username: uniqueUsername, passwordHash: LICENSE_ADMIN_PASSWORD_HASH, role: 'admin', createdAt: ts, updatedAt: ts }; data.users.push(user); await writeJsonAtomic(usersPath(), data); return user }) },
